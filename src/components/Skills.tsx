...
const Skills: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-20 relative overflow-hidden section-gradient-4">
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
          {skillCategories.map((category, index) => (
            <div
              key={index}
              className="group relative bg-gradient-to-br from-violet-500/10 to-purple-600/10 rounded-2xl p-8 border border-violet-500/20 hover:border-violet-500/40 transition-all duration-300 hover:scale-105 scale-in"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${category.color} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 glow`}>
        ...
        {/* Certifications */}
        <div className="mb-20">
          <h3 className="text-3xl font-bold text-center gradient-text mb-12 fade-in">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {certifications.map((cert, index) => (
              <div
                key={index}
                className="text-center p-8 bg-gradient-to-br from-violet-500/10 to-purple-600/10 rounded-2xl border border-violet-500/20 hover:border-violet-500/40 transition-all duration-300 hover:scale-105 scale-in"
                style={{ animationDelay: `${index * 0.2}s` }}
              >
                <div className="w-16 h-16 bg-gradient-to-br from-violet-500 to-purple-600 rounded-2xl flex items-center justify-center mx-auto mb-6 glow">
            ...
            {[
              ...
              >
                <div className="w-1/2 px-8">
                  <div className={`p-8 bg-gradient-to-br from-violet-500/10 to-purple-600/10 rounded-2xl border border-violet-500/20 glow hover:scale-105 transition-transform duration-300 ${
                    index % 2 === 0 ? 'text-right' : 'text-left'
                  }`}>
                    <div className="text-violet-400 font-bold text-xl mb-3">{item.year}</div>
                    <h4 className="text-2xl font-bold text-white mb-2">{item.title}</h4>
                    <div className="text-gray-300 font-semibold mb-4">{item.company}</div>
                ...
                <div className="absolute left-1/2 transform -translate-x-1/2 w-8 h-8 bg-gradient-to-br from-violet-500 to-purple-600 rounded-full border-4 border-black glow" />
